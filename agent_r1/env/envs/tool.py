...
class ToolEnv(AgentEnv):
    ...
    async def step(self, action: Action) -> tuple[Observation, float, bool, dict[str, Any]]:
        ...
        async def _execute_one(
            ...
            tool_response, reward_score, _ = await tool_obj.run(tc.arguments, tools_kwargs=self.tools_kwargs)
            return tool_response.text or "", reward_score, tool_obj.terminal
        ...
        for obs_text, reward_score, is_terminal in results:
        ...
        return Observation(messages=list(self._messages)), total_reward, terminal_triggered, {}
