...
def _fix_json_escapes(s: str) -> str:
    """Fix invalid JSON escape sequences (e.g. \\d, \\s from regex/LaTeX in LLM output)."""
        ...
        for match in matches:
            try:
                try:
                    ...
                    function_call = json.loads(_fix_json_escapes(match))
                name, arguments = function_call["name"], function_call["arguments"]
                if not isinstance(arguments, dict):
                    try:
                        arguments = json.loads(arguments) if isinstance(arguments, str) else {}
                    except json.JSONDecodeError:
                        arguments = json.loads(_fix_json_escapes(arguments)) if isinstance(arguments, str) else {}
        ...
        for match in matches:
            try:
                ...
                try:
                    ...
                    arguments = json.loads(_fix_json_escapes(arguments_str))
                if not isinstance(arguments, dict):
                    arguments = {}
                function_calls.append(ToolCallAction(name=name, arguments=arguments))
            except Exception as e:
