...
class AgentEnvLoop(AgentFlowBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentFlowOutput:
        ...
        for step_idx in range(self.max_steps):
            ...
            # - Intermediate steps with reward=None: coerce to 0.0 (no contribution)
            # - Terminal step (done=True) with reward=None: keep None so that
            #   _compute_score() falls back to custom_reward_function (boxed extraction).
            # - Any step with explicit reward (e.g. from submit_math_answer tool):
            #   use it directly.
            if reward is not None:
                step_reward = float(reward)
            elif done:
                # Terminal step, no tool-level reward → let reward_loop_worker score it
                step_reward = None
            else:
                # Intermediate step with no tool reward (e.g. python_math_executor call)
                step_reward = 0.0
