...
def truncate_dataproto_to_divisor(data: DataProto, divisor: int) -> tuple[DataProto, int]:
    """Drop trailing samples so batch size becomes divisible by ``divisor``.

    This is used as a last-resort guard before actor updates in the new engine
    path, where fixed mini-batch iteration requires exact divisibility.

    Args:
        data: Input batch.
        divisor: Required divisor for the batch size.

    Returns:
        A tuple of ``(possibly_truncated_data, dropped_count)``.
    """
    if divisor <= 0:
        return data, 0
    ...
    keep_size = batch_size - remainder
    if keep_size <= 0:
        raise ValueError(f"Cannot truncate batch of size {batch_size} to divisor {divisor}")

    keep_idxs = torch.arange(keep_size, device=data.batch.device if data.batch is not None else None)
    return data.select_idxs(keep_idxs), remainder
...
class RayAgentTrainer(RayPPOTrainer):
    ...
    def _compute_or_extract_reward(self, batch, reward_fn=None, return_dict=False):
        """Compute reward using reward_fn or extract pre-computed rm_scores."""
        ...
        assert reward_fn is not None, "reward_fn is required when rm_scores not in batch"
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        with marked_timer("update_actor", timing_raw, color="red"):
                            ...
                            batch, dropped_samples = truncate_dataproto_to_divisor(batch, actor_mini_batch_size)
                            if dropped_samples > 0:
                                metrics["actor/truncated_samples"] = dropped_samples
                                print(
                                    "Truncated "
                                    f"{dropped_samples} samples before actor update to make batch size divisible by "
                                    f"{actor_mini_batch_size}."
                                )
    ...
    def _pad_dataproto_to_world_size(self, batch):
        ...
        # Ensure the per-GPU local batch size is also divisible by micro_batch_size_per_gpu.
        # prepare_micro_batches (static path) requires:
        #   local_batch_size % micro_batch_size_per_gpu == 0
        # where local_batch_size = total / dp_size  (dp_size <= world_size).
        # Multiplying the padding divisor by micro_batch_size_per_gpu guarantees this.
        actor_cfg = OmegaConf.select(self.config, "actor_rollout_ref")
        if actor_cfg is not None:
            # infer path (actor): compute_log_prob
            log_prob_mbs = OmegaConf.select(actor_cfg, "rollout.log_prob_micro_batch_size_per_gpu") or 1
            world_size = math.lcm(world_size, int(log_prob_mbs))
            # infer path (ref model): compute_ref_log_prob
            ref_log_prob_mbs = OmegaConf.select(actor_cfg, "ref.log_prob_micro_batch_size_per_gpu") or 1
            world_size = math.lcm(world_size, int(ref_log_prob_mbs))
            # train path: update_actor
            ppo_mbs = OmegaConf.select(actor_cfg, "actor.ppo_micro_batch_size_per_gpu") or 1
            if ppo_mbs:
                world_size = math.lcm(world_size, int(ppo_mbs))
